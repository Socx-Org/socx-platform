...
**Pending.** Proposed direction, for ratification: a system calling another authenticates with a credential **scoped to that specific caller→callee pair** — never a shared platform-wide credential — sourced per `ADR-130` / `SEC-010`. Because `ADR-120` introduces a shared OIDC provider, the preferred realisation is **provider-issued service identities via the OAuth2 client-credentials grant**, so service-to-service and end-user trust share one identity system rather than two.

## Alternatives Considered

- **Per-pair scoped credentials via the shared OIDC provider (client-credentials) — recommended** — least blast radius, reuses `ADR-120`'s provider, centrally revocable and auditable.
