...
| Environment tier | `prod-lab-01` still serves as production with no non-production tier under this repository's management (`OPS-010.1` exception, `ADR-180`, remains open). A full DigitalOcean account query during `reference/terraform`'s real `import` (2026-08-07) found a second, previously-undocumented droplet — `dev-lab-01` (id `564856486`, `139.59.188.136`, `lon1`, `s-1vcpu-1gb`, Ubuntu 24.04 LTS — an older release than `prod-lab-01`'s confirmed 26.04), strong circumstantial evidence it was the surviving remnant of the pre-`ADR-180` shared droplet. **Decommissioned by the platform owner, 2026-08-08**, directly via the DigitalOcean portal, after being left deliberately untriaged since discovery. `OPS-010.1`'s exception remains open — this closes off `dev-lab-01` as a candidate non-production tier, it does not resolve the exception | Observed (B0 attestation for `prod-lab-01`; `dev-lab-01` discovered via DigitalOcean API query, 2026-08-07; decommissioning confirmed by the platform owner, 2026-08-08) |
...
| 1.3     | 2026-08-08 | `dev-lab-01` decommissioned by the platform owner, directly via the DigitalOcean portal, after being left deliberately untriaged since discovery. `OPS-010.1`'s exception remains open | Socx   |
