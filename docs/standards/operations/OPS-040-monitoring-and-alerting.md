...
Applies to the minimum observability required before a SOCX project goes live. Does not cover incident response procedure itself — that belongs in `docs/runbooks/`.
...
3. `OPS-040.3` — Logs MUST be structured, not free-text only, and MUST NOT contain secrets (per `SEC-010.5`).
