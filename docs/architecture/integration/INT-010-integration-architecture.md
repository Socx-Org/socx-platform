...
- **Default style:** synchronous HTTPS, routed through the shared edge (`do-nginx-infra`, see `INF-010`) rather than direct system-to-system network access. This keeps every integration point visible in one place instead of a private mesh of internal routes.
...
- Standard(s) this design satisfies: `ENG-040`
- ADR(s) behind this design: none yet
- Reference implementation(s): `reference/nginx` (currently empty)
- Runbook(s): none yet
