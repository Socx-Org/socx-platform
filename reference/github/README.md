...
- **`enforce_admins: true`.** Secure-by-default: administrators are not exempt from `main`'s protections unless a genuine exception is recorded (`GEN-010.9`). **`socx-platform`'s own real application of this file deliberately used `enforce_admins: false` instead** — `ADR-170`'s 2026-08-08 amendment records this as the expected configuration for any genuinely single-contributor SOCX repository, not a `socx-platform`-specific deviation. `ENG-010.6` already anticipates the scenario: in a repository with no second reviewer, `required_approving_review_count >= 1` with `enforce_admins: true` makes the repository unable to merge its own work at all. `false` preserves every other protection (no direct push, no force-push, no unreviewed merge) for anyone else, while giving the actual admin a working bypass — confirmed for real, not assumed: a direct push after protection was applied succeeded only via that bypass, with an explicit warning, and was reverted. A consuming project with more than one real contributor should keep `enforce_admins: true` as written; revisit the moment a genuine second reviewer joins.
- **Branch protection requires the repository to be public, or GitHub Pro.** Found for real applying this to `socx-platform` (a private repository at the time): both the classic branch-protection endpoint and the newer rulesets endpoint reject any protection payload — even the most minimal one — with `403 Upgrade to GitHub Pro or make this repository public`. `ADR-170`'s 2026-08-08 amendment elevates this from a one-repository finding to a standing constraint every SOCX repository will hit.
...
| ENG-010.9 | Usage step 6 — apply protection no later than the first passing CI run on `main` |
| ENG-010.10 | `scripts/check-branch-protection.sh` |
...
- Runbooks: none yet
- Automation: `reference/deployment` (real deploy steps, Deliverable 6.7, now landed); `.github/workflows/ci.yml` (socx-platform's own real, adapted CI — not a copy of `workflows/ci.yml` in this directory, since this repository has no Node/TS/Express application; see that file's own header comment); `scripts/check-branch-protection.sh` (cross-repo compliance verification, `ENG-010.10` — checks every SOCX repository's *actual* configuration against the mandatory baseline, not just this one)
