...
- **`deploy-release.sh`'s automatic rollback does not cover a Python worker's own dependencies.** Found for real building `rms`'s deploy pipeline (2026-08-08, `socx-platform#97`): this script is Node-oriented — it extracts a release, flips a symlink, restarts systemd units, and health-gates the restart, all language-agnostic — but a Python worker's dependencies (a venv, `pip install -r requirements.txt`) aren't part of that extraction/symlink model at all, since a venv isn't relocatable across versioned release directories the way a Node `node_modules` tree can be. `rms`'s own pipeline works around this with a separate step, updating a persistent venv from the new commit's `requirements.txt` *before* calling `deploy-release.sh`, so a bad dependency change fails before the release ships — but a worker-specific runtime failure *after* that point doesn't trigger this script's rollback path the same principled way an API health-check failure does. Not fixed here: no second real Python-worker deployment exists yet to validate a general solution against: extending this script to understand venvs would be solving a one-example problem speculatively. Recorded so the next Python worker (if any) doesn't have to rediscover this from scratch.
