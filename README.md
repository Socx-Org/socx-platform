# SOCX Platform

This repository is the engineering source of truth for the SOCX platform.

Application code does not belong here.

See [docs/project/project-charter.md](docs/project/project-charter.md) for full purpose, vision, engineering principles, roadmap, and related repositories.

## Contents

- architecture documentation
- current-state documentation
- engineering standards
- deployment standards
- Architecture Decision Records (ADRs)
- reference implementations
- reusable templates
- operational runbooks

`reference/` holds canonical, verified implementations demonstrating how the standards and architecture are realised — complete, parameterised working configuration that consuming repositories copy and adapt (see [reference/README.md](reference/README.md)).

`examples/` holds fully filled-out, worked instances of the content types above (e.g. a completed ADR, a populated runbook) — for reference and onboarding. This is distinct from `templates/`, which holds blank scaffolds meant to be copied and filled in.

This repository's own GitHub process, including a recorded `GEN-010.9` exception for how it currently handles commits to `main`, is documented in [docs/development/github-workflow.md](docs/development/github-workflow.md).

## Roadmap

Current maturity

✓ Standards

✓ Architecture

✓ Current-state

✓ Architecture Decision Records (ADRs)

🚧 Reference implementations (in progress)

⏳ Runbooks

⏳ Automation
